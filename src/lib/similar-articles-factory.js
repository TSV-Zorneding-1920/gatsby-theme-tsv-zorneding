...
export class SimilarArticlesFactory {
  ...
  getArticles() {
    const { category, tags, articles } = this;
    ...
    let arrayIdentityMap = getIdentityMapAsArray();

    // Filter non-matching.
    arrayIdentityMap = arrayIdentityMap.filter(function(article) {
      return article.points > 0;
    });
